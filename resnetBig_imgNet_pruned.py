...
class BasicBlock(nn.Module):
    ...
    def incompatible_sum(self,skip_x,out):
        if self.pruned_filters_conv2!=[] or self.pruned_downsample !=[]:
            device='cpu'
            if torch.cuda.is_available():
                device= 'cuda:0'
            # breakpoint()
            original_num_filters=len(self.pruned_filters_conv2)+self.conv2.weight.size(0)
            combined_unpruned= [i for i in range(original_num_filters) if i not in self.pruned_filters_conv2 and i not in self.pruned_downsample ]
            combined_pruned= [i for i in range(original_num_filters) if i in self.pruned_filters_conv2 and i in self.pruned_downsample ]
            downsample_only_unpruned= [i for i in range(original_num_filters) if i in self.pruned_filters_conv2 and i not in self.pruned_downsample ]
            conv2_only_unpruned= [i for i in range(original_num_filters) if i not in self.pruned_filters_conv2 and i in self.pruned_downsample ]
            
            if combined_unpruned !=[]:
                comb_unpr_conv2 = mask_idx_list(combined_unpruned,self.pruned_filters_conv2)
                comb_unpr_downsample = mask_idx_list(combined_unpruned,self.pruned_downsample)

                out_aux = out[:,comb_unpr_conv2,:]
                skip_x_aux = skip_x[:,comb_unpr_downsample,:]
                out_aux += skip_x_aux
            
                final_idxs_aux = mask_idx_list(combined_unpruned,combined_pruned)

            if downsample_only_unpruned !=[]:
                final_idxs_skip_x = mask_idx_list(downsample_only_unpruned,combined_pruned)
                downsample_only_unpruned = mask_idx_list(downsample_only_unpruned,self.pruned_downsample)
            ...
            if downsample_only_unpruned !=[]:
                t_aux[:,final_idxs_skip_x,:]=skip_x[:,downsample_only_unpruned,:]
...
class Bottleneck(nn.Module):
    ...
    def incompatible_sum(self,skip_x,out):
        if self.pruned_filters_conv3!=[] or self.pruned_downsample !=[]:
            ...
            combined_unpruned= [i for i in range(original_num_filters) if i not in self.pruned_filters_conv3 and i not in self.pruned_downsample ]
            combined_pruned= [i for i in range(original_num_filters) if i in self.pruned_filters_conv3 and i in self.pruned_downsample ]
            downsample_only_unpruned= [i for i in range(original_num_filters) if i in self.pruned_filters_conv3 and i not in self.pruned_downsample ]
            conv3_only_unpruned= [i for i in range(original_num_filters) if i not in self.pruned_filters_conv3 and i in self.pruned_downsample ]
    ...
    def conv_batchnorm_blocks(self):
        ...
        for b in self.modules():
            if isinstance(b, BasicBlock):
                ...
                i+=2
                if isinstance(b.downsample,nn.Sequential) and len(b.downsample)>0 :
                    blocks_list.append( {'conv':b.downsample[0],'batchnorm':b.downsample[1], 'id':i})
                    i+=1
